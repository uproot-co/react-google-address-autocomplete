...
type BaseProps = Pick<
  InputProps,
  'styleType' | 'className' | 'placeholder'
> & {
  name?: string;
  label?: string;
  required?: boolean;
 ...
 *
 */
const AddressAutocompleteComponent: React.FC<
  ConnectedComponentProps | UnconnectedComponentProps
> = ({
  name = 'auto_complete',
  label,
  isConnected,
  ...
  const connectedFieldValue = isConnected
  ...
  useEffect(() => {
    Boolean(inputValue) &&
      fetchAddresses({ variables: { query: inputValue } });
  }, [inputValue, fetchAddresses]);

  useEffect(() => {
    const predictions = get(
      data,
      'data.fetchAutoCompleteAddresses',
      []
    );
